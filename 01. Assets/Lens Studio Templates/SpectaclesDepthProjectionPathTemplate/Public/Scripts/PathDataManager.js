...
function createPathData(option) {
    ...
    if (amountOfFrameToRemove != 0) {
        ...
        counter = 0;
        for (var j = lowerValue; j < biggerValue; j += amountOfFrameToRemove) {
            customPath[counter] = copyIndexData(pathData[j]);
            counter++;
        }
    } else {
        ...
        for (var k = 0; k < frameAmount; k++) {
            customPath[k] = copyIndexData(pathData[counter]);
            counter++;
        }
    }
