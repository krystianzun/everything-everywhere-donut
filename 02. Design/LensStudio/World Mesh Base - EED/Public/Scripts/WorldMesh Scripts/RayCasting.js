...
function spawnOnRaycastResult(rayCastRes) {
    ...
    if (script.spawner.api.instancedSpawners && script.spawner.api.instancedSpawners[classification] && script.spawner.api.instancedSpawners[classification].length > 0) {
        var spawnerIdx = getRandomInt(script.spawner.api.instancedSpawners[classification].length);

        if (script.spawner.api.instancedSpawners[classification][spawnerIdx] && script.spawner.api.instancedSpawners[classification][spawnerIdx].maxInstanceCountReached()) {
            return false;
        }

        var spawnerObject = script.spawner.api.instancedSpawners[classification][spawnerIdx];
        if (!spawnerObject) {
            return false;
        }
        ...
        var transform = script.spawner.api.transformGetters[classification][spawnerIdx].getTransform(pos, normal);
