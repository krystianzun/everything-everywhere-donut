...
// @input float minScale = 0.5 {"widget":"slider", "min":0.01, "max":2.0, "step":0.01}
// @input float maxScale = 0.8 {"widget":"slider", "min":0.01, "max":2.0, "step":0.01}
...
if (script.object) {
    ...
    var curScale = clamp((script.maxScale - script.minScale) * Math.random() + script.minScale, 0.01, 2.0);
    if (!curScale) {
        curScale = 1.0;
    }
...
function addToSpawnersArray(type, spawner) {
    ...
    if (script.spawner.api.instancedSpawners[typeIdx]) {
        script.spawner.api.instancedSpawners[typeIdx].push(spawner);
    }
    
    if (script.spawner.api.transformGetters[typeIdx]) {
        script.spawner.api.transformGetters[typeIdx].push(defaultGetter);
    }
}
